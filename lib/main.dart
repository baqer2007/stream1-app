...
class LocalStorageService {
  ...
  static Future<void> savePlaybackPosition(String id, int positionMs, int durationMs, String title, String poster) async {
    ...
    // تتبع إحصائيات الدقائق المشاهدة
...
class AppSettings extends ChangeNotifier {
  ...
  bool autoSmartDownload = false; // التنزيل التلقائي الذكي

  // إدارة الحسابات والبروفايلات المتعددة
  String? userName;
  String? userEmail;
...
// =========================================================================
// شاشة البحث والفلترة المتقدمة (تصنيف، تقييم، وسنة الإنتاج)
// =========================================================================
      ...
      builder: (_) => StatefulBuilder(
        builder: (ctx, setMState) => Padding(
          ...
          child: Column(
            ...
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
              const Text('فلترة البحث المتقدمة', style: TextStyle(color: Colors.white, fontSize: 16, fontWeight: FontWeight.bold)),
              const SizedBox(height: 12),
              Wrap(
                spacing: 8,
                children: [
                  ChoiceChip(label: const Text('الكل'), selected: _filterType == 'all', onSelected: (_) { setMState(() => _filterType = 'all'); setState(() {}); }),
                  ChoiceChip(label: const Text('أفلام فقط'), selected: _filterType == 'movie', onSelected: (_) { setMState(() => _filterType = 'movie'); setState(() {}); }),
                  ChoiceChip(label: const Text('مسلسلات فقط'), selected: _filterType == 'series', onSelected: (_) { setMState(() => _filterType = 'series'); setState(() {}); }),
                ],
              ),
              const SizedBox(height: 12),
              DropdownButton<String>(
                ...
                value: _filterGenre,
                items: _genres.map((g) => DropdownMenuItem(value: g['key'], child: Text(g['ar']!, style: const TextStyle(color: Colors.white)))).toList(),
                onChanged: (v) {
                  setMState(() => _filterGenre = v!);
                  setState(() {});
                },
              ),
              const SizedBox(height: 12),
              Row(
                children: [
                  const Text('الحد الأدنى للتقييم: ', style: TextStyle(color: Colors.white70)),
                  Text(_minScore.toStringAsFixed(1), style: const TextStyle(color: AppColors.star, fontWeight: FontWeight.bold)),
                ],
              ),
              Slider(
                value: _minScore,
...
// =========================================================================
// شاشة تفاصيل العمل: عرض تصنيفات العمل + قسم الأعمال المشابهة في الأسفل
// =========================================================================
...
class _MediaDetailScreenState extends State<MediaDetailScreen> {
  ...
  void _playEpisode(dynamic ep, int idx) async {
    ...
    // التحميل التلقائي الذكي للحلقة التالية
              ...
              child: Padding(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    // قسم الأعمال المشابهة والمقترحة في الأسفل
...
// =========================================================================
// شاشة الحساب: إحصائيات المشاهدة الشخصية + إدارة البروفايلات + التنزيلات
// =========================================================================
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: TabBarView(
        controller: _tabCtrl,
        children: [
          // 1. التنزيلات مع عداد التقدم الحي والسرعة
          ...
          // 3. قسم إحصائيات المشاهدة الشخصية
          ListView(
            padding: const EdgeInsets.all(16),
            children: [
              Container(
          ...
          // 4. الإعدادات، الحساب، إدارة البروفايلات، واللغة
          ListView(
            padding: const EdgeInsets.all(16),
            children: [
              ...
              SwitchListTile(
                ...
                subtitle: const Text('تنزيل الحلقة التالية ومسح الحلقات المنتهية تلقائياً', style: TextStyle(color: Colors.white54, fontSize: 11)),
